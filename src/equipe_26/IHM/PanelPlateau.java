...
import equipe_26.metier.Dalle;
import equipe_26.metier.Pilier;
...
public class PanelPlateau extends JPanel implements MouseListener, MouseMotionListener {
	...
	public void paintComponent (Graphics g) {
		...
		for (Dalle d : Controleur.getInstance().getPlateau().getEnsDalles()) {
			...
			if (d.getControle() == 'G') {
				g2.drawImage(Toolkit.getDefaultToolkit().getImage("./ressource/images/anneau_" + Controleur.getInstance().getJoueur1().getCoul() + ".png"), d.getX(), d.getY(), this);
			} else if (d.getControle() == 'M') {
				g2.drawImage(Toolkit.getDefaultToolkit().getImage("./ressource/images/anneau_" + Controleur.getInstance().getJoueur2().getCoul() + ".png"), d.getX(), d.getY(), this);
			}

			for (Pilier p : d.getPiliers())
				if (p != null) {
					if(p.getCoul() == 'G') {
						g2.drawImage(Toolkit.getDefaultToolkit().getImage("./ressource/images/pilier_" + Controleur.getInstance().getJoueur1().getCoul() + ".png"), p.getX()-13/2, p.getY()-13/2, this);
					} else {
						g2.drawImage(Toolkit.getDefaultToolkit().getImage("./ressource/images/pilier_" + Controleur.getInstance().getJoueur2().getCoul() + ".png"), p.getX()-13/2, p.getY()-13/2, this);
					}
				}
		}
				...
				if (d.getNom() == btnName.charAt(0)) {
					if (d.getPiliers()[numPilier] == null && d.isConstructible(numPilier)) {
						...
						if( Controleur.getInstance().getPlateau().getNbTour()%2 == 0) {
							g2.drawImage(Toolkit.getDefaultToolkit().getImage("./ressource/images/pilier_" + Controleur.getInstance().getJoueur1().getCoul() + ".png"), 
										d.getX()+33 + d.PILIER_X[numPilier] - 13/2, d.getY()+33 + d.PILIER_Y[numPilier] - 13/2, this);
						} else {
							g2.drawImage(Toolkit.getDefaultToolkit().getImage("./ressource/images/pilier_" + Controleur.getInstance().getJoueur2().getCoul() + ".png"), 
										d.getX()+33 + d.PILIER_X[numPilier] - 13/2, d.getY()+33 + d.PILIER_Y[numPilier] - 13/2, this);
						}
					}
				}
